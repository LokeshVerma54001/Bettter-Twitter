...
const HomeExtras = () => {
  const { getRandomUsers, followUser, user: selfUser } = useUserStore();
  ...
  const handleFollowButton = async (id) => {
    try {
      ...
      if (success) {
        // update UI state after follow/unfollow
    ...
    <div className="hidden flex-col w-[28%] lg:flex items-center">
      <div className="w-80">
        ...
        <div className="w-full border rounded-2xl mt-20 border-gray-600 h-52">
          ...
          <p className="text-sm w-78 mt-3 pl-5">
        ...
        {/* who to follow */}
        <div className="w-full border flex flex-col pb-5 rounded-2xl mt-5 border-gray-600">
          <h1 className="font-extrabold text-xl mt-5 pl-5">Who to follow</h1>
          {randomUsers?.map((randomUser, index) => {
            // check if already followed by logged in user
            const isFollowing =
              randomUser.isFollowed ??
              selfUser?.following?.includes(randomUser._id);
              ...
              >
                ...
                <button
                  onClick={(e) => {
                    ...
                    handleFollowButton(randomUser._id);
                  }}
                  className={`self-center mr-5 h-8 w-24 hover:cursor-pointer rounded-2xl font-semibold ${
                    isFollowing
